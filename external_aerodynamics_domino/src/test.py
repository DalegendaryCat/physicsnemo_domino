...
from physicsnemo.utils.neighbors.knn import knn
...
def main(cfg: DictConfig):
    ...
    for count, dirname in enumerate(dirnames_per_gpu):
        ...
        tag = dirname  # use full dirname as tag
        stl_path = os.path.join(filepath, f"{dirname}.stl")
        vtp_path = os.path.join(filepath, f"{dirname}.vtp")
        vtu_path = os.path.join(filepath, f"{dirname}.vtu")
        vtp_pred_save_path = os.path.join(pred_save_path, f"{dirname}_predicted.vtp")
        vtu_pred_save_path = os.path.join(pred_save_path, f"{dirname}_predicted.vtu")
        ...
        inlet_vel = global_params_reference["inlet_velocity"]
        stream_velocity = float(np.sqrt(sum(v**2 for v in inlet_vel)))  # magnitude = 2.0
        ...
        for key in global_params_types.keys():
            if key == "inlet_velocity":
                for v in inlet_vel:
                    global_params_values_list.append(v)
        ...
        if model_type == "surface" or model_type == "combined":
            ...
            if cfg.model.num_neighbors_surface > 1:
                ...
                surface_coordinates_cpu = surface_coordinates.cpu()
                ii, dd = knn_scipy(
                    surface_coordinates_cpu,
                    surface_coordinates_cpu,
                    k=cfg.model.num_neighbors_surface,
                )
                ii = ii.to(dist.device)
                dd = dd.to(dist.device)
