...
def inference_epoch(
    ...
    for i_batch, sample_batched in enumerate(dataloader.dataset):
        ...
        #     pred_pressure = stl_center_results[0][:, 0]
        #     pred_u_x = stl_center_results[0][:, 1]
        #     pred_u_y = stl_center_results[0][:, 2]
        #     pred_u_z = stl_center_results[0][:, 3]

        # if volume_results is not None:
        #     vol = volume_results[0]
        #     # logger.info(
        #     #     f"[{dirname}]  vol  Ux [{vol[:,0].min().item():.3f}, {vol[:,0].max().item():.3f}]  "
